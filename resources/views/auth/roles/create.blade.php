    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="d-grid gap-2">
                <a class="btn btn-outline-secondary" href="{{ route('role.index') }}"> <i class="fa fa-bars"></i> Back To List</a>
            </div>
            <div class="m-auto">
                
        <input type="hidden" name="model" value="roles">
                <form action="{{ route('role.add') }}" id="role-form" method="POST" enctype="multipart/form-data">
                    @csrf
                        ...
                        @foreach ($permissions as $permission)
                            @if($permission->code == 'admin')
                                <div class="col-md-12">
                                    <div class="permission-group m-2 shadow" code="admin">
                                        <div>
                                            <input type="checkbox" class="form-check-input"  role="permission" name="permissionId[]" code="admin" value="{{ $permission->id }}">
                                        </div>
                                        <div>
                                            <h6 class="w-100 mb-0">{{ Str::replaceFirst('admin, ', '', $permission->code)}}</h6>
                            ...
                            <div class="col-md-4 col-xl-3">
                                <div class="permission-group m-2 shadow">
                                    <div>
                                        <input type="checkbox" class="form-check-input" role="permission" name="permissionId[]" value="{{ $permission->id }}">
                                    </div>
                                    <div>
                                        <h6 class="w-100 mb-0">{{ Str::replaceFirst('admin, ', '', $permission->code)}}</h6>
                                        <p class="small text-muted">{{ $permission->descryption }}</p>
